...
class TensorRequisite {
 public:
   ...
   *
   * Limitation 1. Blocking dims should be dense. Dims marked like 8c, 4o, and others use natural
   *               strides.
  ...
  TensorRequisite TreatAs(const std::string& layout, std::string desired_logic_layout = "") const {
    if (desired_logic_layout.empty()) desired_logic_layout = utils::DefaultLogicLayoutFor(layout);
